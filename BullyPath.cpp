...
void BullyPath::add_frame(int yaw, float speed, Vec3f &intended_position, Vec3f &next_position, int state) {
...
bool BullyPath::advance_frame() {
	...
	if (current_state == STATE_LAVA_DEATH) {
		...
		add_frame(current_yaw, current_speed, next_position, next_position, STATE_LAVA_DEATH);
	}
	else {
		...
		if (abs(x_mod) < 8192 && abs(z_mod) < 8192) {
			...
			if (floor_idx != -1) {
				...
				if (floor.normal[1] < 0.5 && floorY > intended_position[1]) {
					...
					add_frame(next_yaw, current_speed, intended_position, next_position, (floor_idx << 4) | STATE_STEEP_FLOOR);
				}
				else {
					...
					if ((int)next_y >= (int)floorY && (int)next_y < (int)floorY + 37) {
						...
						if (next_y == lava_y) {
							add_frame(next_yaw, next_speed, intended_position, next_position, STATE_LAVA_DEATH);
						}
						else {
							add_frame(next_yaw, next_speed, intended_position, next_position, (floor_idx << 4) | STATE_FRICTION_FLOOR);
						}
					}
					else {
						Vec3f next_position = *current_position;
						...
						add_frame(current_yaw, current_speed, intended_position, next_position, STATE_CLEAR);
					}
				}
			}
			else {
			...
			add_frame(next_yaw, current_speed, intended_position, next_position, STATE_OOB);
		}
	}
